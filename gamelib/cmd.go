...
func NewPreviousCommand(engine *Engine) *PreviousCommand {
	return &PreviousCommand{engine: engine}
}

func (pc PreviousCommand) Run(src cmd.Source, output *cmd.Output, _ *world.Tx) {
	m, err := matchFromSource(pc.engine, src)
	if err != nil {
		output.Error(err.Error())
		return
	...
	m.StateSeries().Previous()

	output.Print(text.Colourf(
		"<green>The match has successfully returned to the previous state</green>",
	))
}
...
func NewRewindCommand(engine *Engine) *RewindCommand {
	return &RewindCommand{engine: engine}
}

func (rc RewindCommand) Run(src cmd.Source, output *cmd.Output, _ *world.Tx) {
	m, err := matchFromSource(rc.engine, src)
	if err != nil {
		output.Error(err.Error())
		return
	...
	m.StateSeries().Rewind()

	output.Print(text.Colourf(
		"<green>The match has been rewound to the first state</green>",
	))
}
